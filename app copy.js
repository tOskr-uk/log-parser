...
// * set a lifeSpan variable
// * pass new log entries into an array for processing.
// * check the new entries looking for combat indicators
// * when positive indicator is found note timestamp and index (This is the start point)
// * check each new entry for timestamp and compare with last combat positive entry. 
// * If duration is > lifespan combat encounter is complete.
//      * slice the combat encounter to a new array and export to file (for now)
//      * clear primary array upto end of combat encounter.
...
fs.watchFile(`${path}${read}`,{interval:interval}, ()=>{
    ...
    fs.readFile(`${path}${read}`,{encoding:'utf8'}, (err, data)=> {
        ...
        console.log(`Last record: ${data[index]}`);
        console.log('-------------------------------------');
...
function parseCombatCheck(arr){
    ...
    arr.forEach(e => {
        ...
        // player dmg
        // Adohi hits a Rujarkian raider for 362 crushing damage.
        // an escaped Rujarkian pitfighter hits Adohi for 177 crushing damage.
